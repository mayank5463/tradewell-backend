...
// //   WATCHED_INDICES,
// //   TOP_INDEX_FUNDS,
// // } = require("../config/watchedInstruments"); // UPDATED — added TOP_INDEX_FUNDS
...
// //           const err = new Error(
// //             `Quotes request failed: HTTP ${res.statusCode} — ${body}`,
// //           );
...
// //       ? Number((ltp - netChange).toFixed(2))
// //       : null;
// //   const dayChangePercent =
// //     prevClose > 0 && netChange != null
// //       ? Number(((netChange / prevClose) * 100).toFixed(2))
// //       : 0;
...
// //     } catch (err) {
// //       console.error(
// //         `[MARKET POLL] ❌ Batch ${label} failed permanently:`,
// //         err.message,
// //       );
// //     }
...
// //     console.warn(
// //       "[MARKET POLL] No instrument keys resolved yet — instrument map still loading?",
// //     );
// //     return;
// //   }
...
// //         sampleKey
// //           ? JSON.stringify({ [sampleKey]: quotes[sampleKey] }, null, 2)
// //           : "(empty)",
// //       );
...
// //         console.warn(
// //           `[MARKET POLL] Skipping ${symbol} — missing last_price or net_change`,
// //           {
...
// //     console.error(
// //       "[MARKET POLL] ❌ UPSTOX_ANALYTICS_TOKEN missing — cannot start polling.",
// //     );
// //     return;
// //   }

// //   console.log(
// //     `[MARKET POLL] Starting — polling every ${POLL_INTERVAL_MS / 1000}s.`,
// //   );
// //   pollOnce();
...
//   getAllEquitySymbols,
// } = require("./instrumentMapService");
// const { WATCHED_INDICES, TOP_INDEX_FUNDS } = require("../config/watchedInstruments");
...
// // NEW — SENSEX resolves fine against the instrument master because its
// // stored trading symbol matches "SENSEX" exactly. NIFTY50 (and every
// // other Nifty sector index) was silently failing getInstrumentKey()
// // lookup — most likely because Upstox's instrument master stores index
// // trading symbols with spaces/mixed case ("Nifty 50", "Nifty Bank", ...)
// // rather than the compact "NIFTY50" form used in watchedInstruments.js.
// // A failed lookup here doesn't throw — it just means the symbol never
// // makes it into symbolToKey, so it's silently dropped from every poll
// // forever, with nothing in the logs to explain why.
// //
// // This tries a short list of likely spellings per index and uses
// // whichever one actually resolves. It's a best-effort patch — if none
// // of these match your instrument master's actual naming, the real fix
// // is in instrumentMapService.js itself, which I don't have visibility
// // into yet.
// const INDEX_SYMBOL_ALIASES = {
//   NIFTY50: ["NIFTY50", "NIFTY 50", "Nifty 50", "NIFTY"],
//   SENSEX: ["SENSEX", "BSE SENSEX", "Sensex"],
//   NIFTYBANK: ["NIFTYBANK", "NIFTY BANK", "Nifty Bank", "BANKNIFTY"],
//   NIFTYIT: ["NIFTYIT", "NIFTY IT", "Nifty IT"],
//   NIFTYFMCG: ["NIFTYFMCG", "NIFTY FMCG", "Nifty FMCG"],
//   NIFTYAUTO: ["NIFTYAUTO", "NIFTY AUTO", "Nifty Auto"],
//   NIFTYPHARMA: ["NIFTYPHARMA", "NIFTY PHARMA", "Nifty Pharma"],
//   NIFTYMETAL: ["NIFTYMETAL", "NIFTY METAL", "Nifty Metal"],
//   NIFTYENERGY: ["NIFTYENERGY", "NIFTY ENERGY", "Nifty Energy"],
//   NIFTYREALTY: ["NIFTYREALTY", "NIFTY REALTY", "Nifty Realty"],
// };

// function resolveIndexInstrumentKey(symbol) {
//   const candidates = INDEX_SYMBOL_ALIASES[symbol] || [symbol];
//   for (const candidate of candidates) {
//     const key = getInstrumentKey(candidate);
//     if (key) return key;
//   }
//   return null;
// }
...
//   // Equities keep using the existing exact-match lookup — that path
//   // already works fine for everything except indices.
//   for (const symbol of getAllEquitySymbols()) {
//     const instrumentKey = getInstrumentKey(symbol);
//     if (instrumentKey) {
//       symbolToKey[symbol] = instrumentKey;
//       keyToSymbol[instrumentKey] = symbol;
...
//   // Indices go through the alias-aware resolver.
//   const unresolvedIndices = [];
//   for (const symbol of indexUniverse) {
//     const instrumentKey = resolveIndexInstrumentKey(symbol);
//     if (instrumentKey) {
//       symbolToKey[symbol] = instrumentKey;
//       keyToSymbol[instrumentKey] = symbol;
//     } else {
//       unresolvedIndices.push(symbol);
//     }
//   }

//   if (unresolvedIndices.length > 0) {
//     console.warn(
//       `[MARKET POLL] Could not resolve instrument key for: ${unresolvedIndices.join(", ")}. ` +
//       `These will be missing from /market/indices and /market/index-funds until instrumentMapService's ` +
//       `symbol naming is confirmed and INDEX_SYMBOL_ALIASES updated to match.`,
//     );
//   }
