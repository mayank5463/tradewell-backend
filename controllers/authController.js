...
// ══════════════════════════════════════════════════════════════════════
// PRODUCTION FIX: all four of these used to be in-memory Map/Set
// instances living in server RAM (`tokenBlacklist`, `failedLogins`,
// `resetTokens`, `verificationTokens`). That meant every Render
// restart/redeploy silently wiped them — logged-out tokens became
// valid again, account lockouts reset, and any pending password-reset
// or email-verification link a user hadn't clicked yet died instantly
// even though the email promised it was valid for 24h/30min.
//
// They're now backed by Mongo (see models/PasswordResetToken.js,
// models/VerificationToken.js, models/BlacklistedToken.js) with TTL
// indexes so expired entries clean themselves up automatically.
//
// Account lockout no longer needs its own store at all — UserModel.js
// already had `loginAttempts` / `accountLockedUntil` fields and
// matching instance methods (`recordFailedLogin`, `isAccountLocked`,
// `clearFailedLoginAttempts`) that were sitting unused. We just call
// those directly now instead of reinventing them with a Map.
// ══════════════════════════════════════════════════════════════════════

// ── HELPER: Add token to blacklist ─────────────────────────────────────
async function blacklistToken(token) {
  try {
    // Matches the JWT's own "7d" lifetime from issueAuthCookie.js — no
    // point blacklisting a token past the point it would fail
    // jwt.verify() on its own.
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    await BlacklistedTokenModel.create({ token, expiresAt });
  } catch (err) {
    // If this token was somehow already blacklisted (duplicate key),
    // that's fine — the end state we want is already true.
    if (err.code !== 11000) {
      console.error("[BLACKLIST] Failed to store token:", err.message);
    }
  }
}
...
async function isTokenBlacklisted(token) {
  const found = await BlacklistedTokenModel.findOne({ token }).lean();
  return !!found;
}
...
async function signup(req, res) {
  ...
  try {
    ...
    // ── Generate Email Verification Token (persisted, not in-memory) ───
    const verificationToken = crypto.randomBytes(32).toString("hex");
...
// ══════════════════════════════════════════════════════════════════════
// POST /login - Advanced Login with Persisted Account Lockout
// ══════════════════════════════════════════════════════════════════════
async function login(req, res) {
  const { email, password } = req.body;
  ...
  try {
    ...
    // explicitly requested here or bcrypt.compare() has nothing to
    // check against.
    ...
    if (!user) {
      // No user to record an attempt against — just reject generically
      // so we don't leak which emails exist.
      return res.status(400).json({ message: "Invalid email or password." });
    }

    // ── Check Account Lockout (persisted on the user document) ────────
    ...
    // ── Clear Failed Attempts on Success ───────────────────────────────
    await user.clearFailedLoginAttempts();
    console.log("[LOGIN] ✅ User logged in:", user.email);
...
// ══════════════════════════════════════════════════════════════════════
// POST /logout - Logout with Persisted Token Blacklist
// ══════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════
// GET /check-auth - Enhanced with Persisted Blacklist Check
// ══════════════════════════════════════════════════════════════════════
async function checkAuth(req, res) {
  ...
  // Check if token is blacklisted (e.g. user logged out on another tab)
  const token = req.cookies?.token;
...
// ══════════════════════════════════════════════════════════════════════
// POST /forgot-password - Secure Password Reset Request (Persisted)
// ══════════════════════════════════════════════════════════════════════
async function forgotPassword(req, res) {
  const { email } = req.body;
  ...
  try {
    ...
    // ── Store hashed token in Mongo, NEVER the plain text ──────────────
...
// ══════════════════════════════════════════════════════════════════════
// POST /reset-password - Reset Password with Token (Persisted)
// ══════════════════════════════════════════════════════════════════════
async function resetPassword(req, res) {
  const { token, newPassword } = req.body;
  ...
  try {
    // ── Hash the Token for Comparison ──────────────────────────────────
    const hashedToken = crypto.createHash("sha256").update(token).digest("hex");
    const tokenDoc = await PasswordResetTokenModel.findOne({ hashedToken });

    // ── Validate Token ─────────────────────────────────────────────────
    // Note: no manual expiry check needed here — the TTL index means an
    // expired document is already gone from Mongo, so "not found" and
    // "expired" collapse into the same lookup. We keep a friendly,
    // single message for both cases.
    if (!tokenDoc) {
      console.warn("[RESET-PASSWORD] Invalid or expired token attempted");
      return res.status(400).json({
...
// ══════════════════════════════════════════════════════════════════════
// GET /verify-email/:token - Verify Email Address (Persisted)
// ══════════════════════════════════════════════════════════════════════
async function verifyEmail(req, res) {
  const { token } = req.params;
  ...
  try {
    ...
    // Same reasoning as resetPassword: the TTL index already removes
    // expired documents, so "not found" covers both invalid and
    // expired links.
    if (!tokenDoc) {
      return res.status(400).json({
...
// ══════════════════════════════════════════════════════════════════════
// POST /resend-verification - Resend Verification Email (Persisted)
// ══════════════════════════════════════════════════════════════════════
async function resendVerification(req, res) {
  const { email } = req.body;
  ...
  try {
    ...
    // Invalidate any older outstanding tokens for this user before
    // issuing a new one, so an old email link can't linger alongside
    // a fresh one.
